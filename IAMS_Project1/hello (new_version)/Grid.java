...
public class Grid extends Agent {
    ...
    private int tries=0;
    private int maxTries=100;  //number of actions in an episode | each agent makes a total of maxTries actions
    ...
    public static int[][][][] heuristics = new int[numberOfAgents][4][x][y];
        ...
        addBehaviour(new CyclicBehaviour(this) {  //runs until the grid shuts down | loops for maxTries times (tries=0,1,2,3,4)
            ...
            public void action(){
                ...
                if (tries<maxTries) {
                    ...
                    /*************************************************************  MESSAGE: MAKE MOVE  **************************************************************************/
                    for (int i = 0; i < numberOfAgents; i++) {  //the grid says outloud that some agent just moved in a particular direction
...
//                    System.exit(0);
	...
	private void makeMove(int id, String content, String[][] grid) {
    	...
    	possible = changePositions(type, currentPosition, nextPosition, agId, grid, id);
		...
		if (availability[id] == false && possible) {
			for (int cl = 0; cl < clients.size(); cl++) {
				...
				if (agentsXclients[id][cl] == true) {
					...
					possible = changePositions(type, currentPosition, nextPosition, clId, grid, id);
					break;
				}
			}
		}
    ...
    public String coordinatesToDirections(int[] curr, int[] next) {  // returns the direction according to the current position & the next position
    ...
    public boolean changePositions (String t, int [] currentPos, int [] nextPos, String name, String[][] gr, int agId) {
				...
				for (int j1 = 0; j1 < y; j1++) {
					...
					for (int i2 = 0; i2 < x; i2++) {
						for (int j2 = 0; j2 < y; j2++) {
							...
							if (i1 == current[0] && j1 == current[1] && i2 == next[0] && j2 == next[1]) { /* the cell behind the wall is no
								...
								if (!AStarAlgorithm.isForbidden(drct, thisPos)) {
									neighbors[dirId] = heuristics[id][AStarAlgorithm.getIndex(colours, MyAgent.endBlock[id])][i2][j2];
								}
					...
					for (int n = 0; n < 4; n++) {  //CASE: index out of bounds when adjacentPos =! nextPos | gets heuristic value of this position plus ONE
						if (neighbors[n] == -999999) {
							neighbors[n] = heuristics[id][AStarAlgorithm.getIndex(colours, MyAgent.endBlock[id])][thisPos[0]][thisPos[1]] + 1;
						}
					}
					
					// changes the heuristic value of cell (currentPos[0], currentPos[1]) due to his bumping into the wall
					int nearestNeighborValue = AStarAlgorithm.getMin(neighbors);
					int oldH = heuristics[id][AStarAlgorithm.getIndex(colours, MyAgent.endBlock[id])][thisPos[0]][thisPos[1]];  //the old heuristic value
					
					if (!(MyAgent.endBlock[id][0] == thisPos[0] && MyAgent.endBlock[id][1] == thisPos[1])) {
						heuristics[id][AStarAlgorithm.getIndex(colours, MyAgent.endBlock[id])][thisPos[0]][thisPos[1]] = nearestNeighborValue + 1;
					}
					int newH = heuristics[id][AStarAlgorithm.getIndex(colours, MyAgent.endBlock[id])][thisPos[0]][thisPos[1]]; //the new heuristic value
